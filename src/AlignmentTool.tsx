...
import { needlemanWunsch, smithWaterman, type AlignmentResult } from "./alignment";
	...
	return (
		<div className="alignment-tool">
			...
			<div className="input-section">
				<textarea placeholder="Enter sequence 1" value={seq1} onChange={e => setSeq1(e.target.value)} />
				<textarea placeholder="Enter sequence 2" value={seq2} onChange={e => setSeq2(e.target.value)} />
			</div>
			<div className="parameters-section">
				<div className="alignment-type">
					<label>
						<input type="radio" value="global" checked={alignmentType === "global"} onChange={() => setAlignmentType("global")} />
						Global (Needleman-Wunsch)
					</label>
					<label>
						<input type="radio" value="local" checked={alignmentType === "local"} onChange={() => setAlignmentType("local")} />
				...
				<div className="scoring-parameters">
					<label>
						Match Score:
						<input type="number" value={matchScore} onChange={e => setMatchScore(Number(e.target.value))} disabled={substitutionMatrix !== "none"} />
					</label>
					<label>
						Mismatch Score:
						<input type="number" value={mismatchScore} onChange={e => setMismatchScore(Number(e.target.value))} disabled={substitutionMatrix !== "none"} />
					</label>
					<label>
						Gap Penalty:
						<input type="number" value={gapPenalty} onChange={e => setGapPenalty(Number(e.target.value))} />
					</label>
				</div>
				<div className="substitution-matrix">
					<label>
						Substitution Matrix:
						<select value={substitutionMatrix} onChange={e => setSubstitutionMatrix(e.target.value)}>
					...
					{substitutionMatrix !== "none" && <p className="info-text">Using {substitutionMatrix} for scoring.</p>}
				</div>
			</div>
